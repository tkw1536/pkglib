...
//spellchecker:words math exec strconv testing pkglib errorsx exit
import (
	"fmt"
	"math"
	"os"
...
func TestExitCode_Return(t *testing.T) {
	...
	for exitCode := range uint8(255) {
		...
		t.Run(exitCodeStr, func(t *testing.T) {
			...
			cmd := exec.CommandContext(t.Context(), os.Args[0], "-test.run="+t.Name()) // #nosec G204, G702 -- we need this for the test
