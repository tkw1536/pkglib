...
func Anyways(parent context.Context, timeout time.Duration) (context.Context, context.CancelFunc) {
	...
	if err := parent.Err(); err == nil {
		return context.WithTimeout(parent, timeout) // #nosec G118 // to be called by parent
	}

	// context is cancelled => create a new one with a custom timeout
	return context.WithTimeout(context.WithoutCancel(parent), timeout) // #nosec G118 // to be called by parent
}
